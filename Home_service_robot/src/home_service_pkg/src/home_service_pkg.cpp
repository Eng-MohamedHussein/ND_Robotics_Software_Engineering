...
{
  ros::init(argc, argv, "add_marker_new");
  ros::NodeHandle n;
  ros::Rate r(1);
  ...
  while (ros::ok())
  {
    ...
    double distance = calculateDistance(pick_up_x, pick_up_y, RobotX, RobotY);
    ...
    
    {
        ...
        marker_pub.publish(marker);
        pickedUp = true;
        pick_up_x=drop_off_x;
        pick_up_y=drop_off_y;

        marker.action = visualization_msgs::Marker::ADD;
    }
            

    distance = calculateDistance(pick_up_x,pick_up_y, RobotX, RobotY);
